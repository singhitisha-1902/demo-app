...
import { getOptionsGroups } from './get-options-groups.esm.js';
...
function isItemIncludesSearchTerm(item, searchTerm) {
  return item.label?.toLowerCase().includes(String(searchTerm).toLowerCase());
}
...
const useSelectItems = initialState => {
  const {
    ...
    groupedOptions,
    hasOptionsGroups
  } = getOptionsGroups(initialState.options);
  ...
  const addOption = useCallback(opt => {
    dispatch({
      type: ActionKind.ADD,
      payload: opt
    });
  ...
  const deselectOption = useCallback(opt => {
    dispatch({
      type: ActionKind.DESELECT,
      payload: opt
    });
  }, []);
  const removeOption = useCallback(opt => {
    dispatch({
  ...
  const setSearchValue = useCallback(q => {
    dispatch({
      type: ActionKind.SEARCH,
      payload: q
    });
  }, []);
  const updateSelect = useCallback(opts => {
    dispatch({
